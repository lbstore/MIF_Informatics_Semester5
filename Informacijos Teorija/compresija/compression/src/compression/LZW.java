...
public class LZW
{
    ...
    private static int dictionary[][]=new int[size][size];
    private static int pattern[]=new int[size];
    private static int element[]=new int[size];
    private static int chars[]=new int[256];
    private static int chfreq[]=new int[256];
    private static float chprobs[]=new float[256];
    private static int ofreq[]=new int[size];
    private static int dfree,psize,esize;
    private static int top,tc;
    private static int chnext;

    public static void ini()//function that initializes variables
    {
    ...
    private static float calcEntropy()//function that calculates
    ...
    private static float calcAvCodeLength()//function that calculates average
            //code length
    {
        int t=0;
        int sum=0;
    ...
    private static int inchars(int x)//function that checks whaether a
            //character from input file has been read already
    {
        for(int i=0;i<chnext;i++)
        {
            if(chars[i]==x)return i;
        }

        return -1;
    }

    private static boolean indict(int x)//fnction that checks whether a pattern
            //exists in the dictionary
    {
        ...
        for(int i=0;i<4000;i++)
        {
            if(pattern[0]==dictionary[i][0])
            {
                ...
                {
                    if(pattern[j]==dictionary[i][j])n++;
                }

                if(n==psize)return true;
            }
        }
    ...
    private static void index(DataOutputStream out)throws IOException//function
            //that outputs dictionary index of the pattern
    {
    ...
    private static void add(int x)//function that adds pattern to the dictionary
    {
        try{
    ...
    private static void clear()//function that clears the pattern
    {
        for(int i=0;i<psize;i++)
            pattern[i]=-1;
        psize=0;
    ...
    public static void compress(String infile,String outfile)//function that
            //compresses a file
    {
        try
        {
    ...
    public static void decompress(String infile,String outfile)//function that
            //decompresses a file
    {
        try
        {
            ...
            while((x=in.read())!=-1)
            {
                ...
                if(x>=dfree)
                {
                    for(int i=0;i<esize;i++)element[i]=-1;
                    esize=0;
                    for(int i=0;i<psize;i++)
                    {
                        element[i]=pattern[i];
                ...
                for(int i=0;i<esize;i++)out.write(element[i]);
                for(int i=0;i<psize;i++)dictionary[dfree][i]=pattern[i];
    ...
    public static void main(String args[])
    {
        ...
        System.out.println("2910325 Data Compression coursework 1\n" +
                "by Nikola Pavlicevic\n\n" +
                "Implementation of LZW compression and decompression algorithm\n\n");
